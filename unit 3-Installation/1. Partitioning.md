...
3. **Select the Disk**: Type `list disk` to list all available disks and their numbers. In your case, Disk 2 is the one with Volume 6 that you want to work with. So, type `select disk 2` and press Enter to select Disk 2.

4. **List the Volumes on the Selected Disk**: Type `list volume` to list all volumes on the selected disk. Make sure that Volume 6 is the one you want to shrink.
